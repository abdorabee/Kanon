...
// No longer needed as we're using next-translate
      ...
      >
        ...
        >
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-[#e0e0e0] tracking-wide">
            {t('response.searchResults')}: <span className="text-[#4caf50] bg-[#2e7d32] px-2 py-1 rounded-full text-sm sm:text-base">{issues.length}</span>
          </h1>
            ...
            <table className="w-full border-collapse min-w-[640px]">
              <thead>
                <tr className="border-b border-[#4d4d4d]">
                  <th className="py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm md:text-base font-semibold text-[#e0e0e0] text-left">{t('response.properties')}</th>
                  {/* Column headers are now the property names */}
                  <th className="py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm md:text-base font-semibold text-[#e0e0e0] text-center">{t('response.caseNumber')}</th>
                  <th className="py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm md:text-base font-semibold text-[#e0e0e0] text-center">{t('response.table')}</th>
                  <th className="py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm md:text-base font-semibold text-[#e0e0e0] text-center">{t('response.plaintiff')}</th>
                  <th className="py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm md:text-base font-semibold text-[#e0e0e0] text-center">{t('response.defendant')}</th>
                  <th className="py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm md:text-base font-semibold text-[#e0e0e0] text-center">{t('response.judgment')}</th>
                </tr>
              </thead>
              <tbody>
                ...
                {issues.map((issue, index) => (
                  <tr key={issue._id} className="border-b border-[#4d4d4d] hover:bg-[#3d3d3d] transition-colors">
                    <th className="py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm md:text-base font-semibold text-[#e0e0e0] text-left">{t('response.case')} {index + 1}</th>
                    ...
                    <td className="py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm md:text-base text-center">
                      ...
                      >
                        {expandedIssues[issue._id] ? t('response.hide') : t('response.view')}
                      </button>
                      <AnimatePresence>
        ...
        >
          <Button onClick={() => router.push('/')} translationKey="response.backToHome" />
        </motion.div>
      </motion.div>
    </div>
  );
